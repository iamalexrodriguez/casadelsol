...
router.post("/signup", (req, res, next) => {
  ...
  if (email === "" || password === "") {
    res.render("auth/signup", { message: "Es necesario indicar un email y contraseña" });
    return;
  }

  User.findOne({ email }, "email", (err, user) => {
    if (user !== null) {
      res.render("auth/signup", { message: "Este email ya lo tenemos dado de alta en una cuenta :)" });
      return;
    }
    ...
    .catch(err => {
      res.render("auth/signup", { message: "Algo salio mal al darte de alta" });
    })
  });
});
